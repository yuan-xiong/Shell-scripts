...
int trace_print_context(struct trace_iterator *iter)
{
	...
	// orig: only for children
	//trace_find_cmdline(entry->pid, comm);
	//trace_seq_printf(s, "%16s-%-5d [%03d] ",
	//		       comm, entry->pid, iter->cpu);

	// version 1: for children and parent
	//int ppid = -1;
	//char p_comm[TASK_COMM_LEN];
	//trace_find_cmdline_parent(entry->pid, comm, &ppid, p_comm);
	//trace_seq_printf(s, "%16s-%-5d p:%16s-%5d [%03d] ",
	//		       comm, entry->pid, p_comm, ppid, iter->cpu);

	// version 2: for children, parent and grandparent
	//int ppid = -1, gpid = -1;
	//char p_comm[TASK_COMM_LEN];
	//char g_comm[TASK_COMM_LEN];
	//trace_find_cmdline_grandparent(entry->pid, comm, &ppid, p_comm, &gpid, g_comm);
	//trace_seq_printf(s, "c:%16s-%-5d p:%16s-%-5d g:%16s-%-5d [%03d] ",
	//		       comm, entry->pid, p_comm, ppid, g_comm, gpid, iter->cpu);

	// version 3: for children, parent, grandparent and ggparent
	int ppid = -1, gpid = -1, ggpid = -1;
	char p_comm[TASK_COMM_LEN];
	char g_comm[TASK_COMM_LEN];
	char gg_comm[TASK_COMM_LEN];
	trace_find_cmdline_ggparent(entry->pid, comm, &ppid, p_comm, &gpid, g_comm, &ggpid, gg_comm);
	trace_seq_printf(s, "c:%16s-%-5d p:%16s-%-5d g:%16s-%-5d gg:%16s-%-5d [%03d] ",
			       comm, entry->pid, p_comm, ppid, g_comm, gpid, gg_comm, ggpid, iter->cpu);
